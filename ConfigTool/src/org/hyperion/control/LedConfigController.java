...
public class LedConfigController {
	...
	public LedConfigController(final LedStringModel pModel) {
		...
		mModel.ledFrameConfig.addObserver(mObserver, LedFrameConstructionModel.LEDFRAMECONSTRUCTION_EVENT);
		mModel.processConfig.addObserver(mObserver, ImageProcessModel.IMAGEPROCESSMODEL_EVENT);
	}
	...
	private void updateLeds() { 
		...
		int totalLedCount = mModel.ledFrameConfig.getLedCount();
		if (totalLedCount <= 0) {
		...
		int iLed = (totalLedCount - mModel.ledFrameConfig.firstLedOffset.getValue())%totalLedCount;
		if (iLed < 0) {
			iLed += totalLedCount;
		}
		
		// Construct the top-left led (if top-left is enabled)
		if (mModel.ledFrameConfig.topCorners.getValue()) {
			mModel.leds.add(createLed(iLed, 0.0, 0.0, mModel.processConfig.overlapFraction.getValue(), BorderSide.top_left));
			iLed = increase(iLed);
		}
		
		// Construct all leds along the top of the screen (if any)
		if (mModel.ledFrameConfig.topLedCnt.getValue() > 0) {
			// Determine the led-spacing
			int ledCnt = mModel.ledFrameConfig.topLedCnt.getValue();
			...
			for (int iTop=0; iTop<ledCnt; ++iTop) {
				...
				mModel.leds.add(createLed(iLed, led_x, led_y, mModel.processConfig.overlapFraction.getValue(), BorderSide.top));
				iLed = increase(iLed);
			}
		}
		...
		if (mModel.ledFrameConfig.topCorners.getValue()) {
			mModel.leds.add(createLed(iLed, 1.0, 0.0, mModel.processConfig.overlapFraction.getValue(), BorderSide.top_right));
			iLed = increase(iLed);
		}
		
		// Construct all leds along the right of the screen (if any)
		if (mModel.ledFrameConfig.rightLedCnt.getValue() > 0) {
			// Determine the led-spacing
			int ledCnt = mModel.ledFrameConfig.rightLedCnt.getValue();
			...
			for (int iRight=0; iRight<ledCnt; ++iRight) {
				...
				// Construct and add the single led specification to the list of leds
				mModel.leds.add(createLed(iLed, led_x, led_y, mModel.processConfig.overlapFraction.getValue(), BorderSide.right));
				iLed = increase(iLed);
			}
		}
		
		// Construct the bottom-right led (if bottom-right is enabled)
		if (mModel.ledFrameConfig.bottomCorners.getValue()) {
			mModel.leds.add(createLed(iLed, 1.0, 1.0, mModel.processConfig.overlapFraction.getValue(), BorderSide.bottom_right));
			iLed = increase(iLed);
		}
		
		// Construct all leds along the bottom of the screen (if any)
		if (mModel.ledFrameConfig.bottomLedCnt.getValue() > 0) {
			...
			for (int iBottom=(ledCnt-1); iBottom>=0; --iBottom) {
				...
				if (iBottom > (mModel.ledFrameConfig.bottomLedCnt.getValue()-1)/2 && iBottom < ledCnt - mModel.ledFrameConfig.bottomLedCnt.getValue()/2) {
					continue;
				}
				// Compute the location of this led
				...
				// Construct and add the single led specification to the list of leds
				mModel.leds.add(createLed(iLed, led_x, led_y, mModel.processConfig.overlapFraction.getValue(), BorderSide.bottom));
				iLed = increase(iLed);
			}
		}
		...
		if (mModel.ledFrameConfig.bottomCorners.getValue()) {
			mModel.leds.add(createLed(iLed, 0.0, 1.0, mModel.processConfig.overlapFraction.getValue(), BorderSide.bottom_left));
			iLed = increase(iLed);
		}
		
		// Construct all leds along the left of the screen (if any)
		if (mModel.ledFrameConfig.leftLedCnt.getValue() > 0) {
			// Determine the led-spacing
			int ledCnt = mModel.ledFrameConfig.leftLedCnt.getValue();
			double ledSpacing = (double)1.0/ledCnt;
			...
			for (int iRight=(ledCnt-1); iRight>=0; --iRight) {
				...
				// Construct and add the single led specification to the list of leds
				mModel.leds.add(createLed(iLed, led_x, led_y, mModel.processConfig.overlapFraction.getValue(), BorderSide.left));
				iLed = increase(iLed);
			}
		}
		...
		Collections.sort(mModel.leds, new Comparator<LedModel>() {
			...
			public int compare(LedModel o1, LedModel o2) {
				return Integer.compare(o1.index.getValue(), o2.index.getValue());
			}
		});
	...
	private int increase(int pLedCounter) {
		if (mModel.ledFrameConfig.clockwiseDirection.getValue()) {
			return (pLedCounter+1)%mModel.ledFrameConfig.getLedCount();
		} else {
			if (pLedCounter == 0) {
				return mModel.ledFrameConfig.getLedCount() - 1;
			}
	...
	private LedModel createLed(int seqNr, double x_frac, double y_frac, double overlap_frac, BorderSide pBorderSide) {
		...
		led.index.setValue(seqNr);
		led.side.setValue(pBorderSide);
		
		final double verticalGap = mModel.processConfig.verticalGap.getValue();
		final double horizontalGap = mModel.processConfig.horizontalGap.getValue();
		
		final int topLedCnt  = mModel.ledFrameConfig.topLedCnt.getValue();
		final int leftLedCnt = mModel.ledFrameConfig.leftLedCnt.getValue();
		...
		double horizontalDepth = Math.min(1.0 - horizontalGap, mModel.processConfig.horizontalDepth.getValue());
		double verticalDepth   = Math.min(1.0 - verticalGap,   mModel.processConfig.verticalDepth.getValue());
		...
		led.hscan.minimum.setValue(ledArea.getMinX());
		led.hscan.maximum.setValue(ledArea.getMaxX());
		led.vscan.minimum.setValue(ledArea.getMinY());
		led.vscan.maximum.setValue(ledArea.getMaxY());
