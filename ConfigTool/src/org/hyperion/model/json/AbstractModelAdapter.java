...
import org.hyperion.model.SelectableModelList;
	...
	public JsonElement serialize(AbstractModel pSrc, Type pTypeOfSrc, JsonSerializationContext pContext) {
		...
		for (Field field : fields) {
			...
			if(object instanceof ParameterBool) {
				...
				result.add(parameter.getName(), new JsonPrimitive(parameter.getValue()));
			} else if (object instanceof ParameterString) {
				ParameterString parameter = (ParameterString) object;
				result.add(parameter.getName(), new JsonPrimitive(parameter.getValue()));
			} else if (object instanceof ParameterStringEnum) {
				ParameterStringEnum parameter = (ParameterStringEnum) object;
				result.add(parameter.getName(), new JsonPrimitive(parameter.getValue()));
			...
			} else if (object instanceof OptionalModel) {
				OptionalModel<?> optMod = (OptionalModel<?>) object;
		        ...
		        result.add(mNaming.translateName(field), pContext.serialize(optMod.get(), elementClass));
		...
		for (Field field : fields) {
			...
			if(object instanceof ParameterBool) {
				...
				if (element == null) {
					...
					// TODO[TvdZ]: Check if value is in list and else report error
					parameter.setValue(element.getAsString());
				}
			} else if(object instanceof ParameterString) {
				ParameterString parameter = (ParameterString) object;
				JsonElement element = pJsonObject.get(parameter.getName());
				...
				if (element == null) {
			        ...
			        ParameterizedType elementType = (ParameterizedType) field.getGenericType();
			        final Class<?> elementClass = (Class<?>) elementType.getActualTypeArguments()[0];
					optMod.set(pContext.deserialize(element, elementClass));
				}
